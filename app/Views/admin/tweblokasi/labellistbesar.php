...
<body>
	<h1>RUANGAN</h1>
	<table class="table-list" width="300" border="1" cellspacing="0" cellpadding="4">
		<tr>
	<?php
# JIKA MENEMUKAN CBKODE, salah satu Cekbox dipilih dan klik tombol Cetak Barcode
	if(count($list) == 0) {
		...
		foreach ($list as $key => $value) {
			...
			$query = $db->table('inventaris_peralatan')->where(['idinventaris_peralatan' => $kd])->get()->getResultArray();

			foreach ($query as $key => $value2) {
				$result = Builder::create()
			    ->writer(new PngWriter())
			    ->writerOptions([])
			    ->data($value2['kode_barang'] . "." . $value2['nup'])
				...
				if ($no == $lebar) { echo "</tr>"; $lebar = $lebar + 2; }
			}
		}
	}
?>
